// Memory Mesh - Create memory networks
export default {
    init() {
        return {
            input: '',
            output: null,
            processing: false,
            history: JSON.parse(localStorage.getItem('memory-mesh-history') || '[]'),
            settings: JSON.parse(localStorage.getItem('memory-mesh-settings') || '{}')
        };
    },
    ...
    process(state, setState) {
        ...
        setTimeout(() => {
            ...
            localStorage.setItem('memory-mesh-history', JSON.stringify(state.history));
        }, 1000);
    },
