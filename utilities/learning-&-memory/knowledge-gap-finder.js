// Knowledge Gap Finder - Find knowledge gaps
export default {
    init() {
        return {
            input: '',
            output: null,
            processing: false,
            history: JSON.parse(localStorage.getItem('knowledge-gap-finder-history') || '[]'),
            settings: JSON.parse(localStorage.getItem('knowledge-gap-finder-settings') || '{}')
        };
    },
    ...
    process(state, setState) {
        ...
        setTimeout(() => {
            ...
            localStorage.setItem('knowledge-gap-finder-history', JSON.stringify(state.history));
        }, 800);
    },
