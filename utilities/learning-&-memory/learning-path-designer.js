// Learning Path Designer - Design learning paths
export default {
    init() {
        return {
            input: '',
            output: null,
            processing: false,
            history: JSON.parse(localStorage.getItem('learning-path-designer-history') || '[]'),
            settings: JSON.parse(localStorage.getItem('learning-path-designer-settings') || '{}')
        };
    },
    ...
    process(state, setState) {
        ...
        setTimeout(() => {
            ...
            localStorage.setItem('learning-path-designer-history', JSON.stringify(state.history));
        }, 1000);
    },
