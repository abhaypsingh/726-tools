// Recipe Ratio Calculator - A utility in the Food & Cooking category
export default {
    init() {
        return {
            input: '',
            results: [],
            settings: JSON.parse(localStorage.getItem('recipe-ratio-calculator-settings') || '{}'),
            history: JSON.parse(localStorage.getItem('recipe-ratio-calculator-history') || '[]')
        };
    },
        ...
        if (processBtn) {
            processBtn.addEventListener('click', () => {
                if (state.input) {
                    ...
                    localStorage.setItem('recipe-ratio-calculator-history', JSON.stringify(newHistory));
                    setState({
                        results: [result],
                        history: newHistory
                    });
    ...
    process(input) {
        // Processing logic for Recipe Ratio Calculator
